...
class uncompleteTasks extends StatefulWidget {
  final Taskmodel task; // Define a named parameter 'task'

  const uncompleteTasks({super.key, required this.task}); // Constructor
...
class _uncompleteTasksState extends State<uncompleteTasks> {
  ...
  // Widget build(BuildContext context) {
    // Access the task using widget.task
    //
    ...
    Future<List<Taskmodel>> fetchData() async {
      ...
      try {
        final response = await dio.get(
          '$ip/UnCompleteTask?userid=${loggedInUser!.id}',
        );
    ...
    Widget build(BuildContext context) {
      ...
      return Scaffold(
        appBar: AppBar(
          ...
          title: const Center(child: Text("UnComplete Task")),
          leading: IconButton(
